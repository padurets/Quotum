...
export function useResets(enabled: boolean): {resets: Resets; health: TrackerHealth[]} {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const answer = await call<{resets: Resets; trackers: TrackerHealth[]}>('GET', '/api/resets', undefined, 10_000);
  ...
  return enabled ? {resets: state.resets, health: state.trackers} : {resets: {}, health: []};
}
