 ...
 * the chart draws a plan somewhere in the period (`planned`). A forecast is read in a
 * cell wholly ahead of now, until its window runs out; its column is there when the
 * chart draws one (`foreseen`), with the plan or without it.
 */
export function readout(lines: Line[], plans: PlanLine[], cell: number, cellMs: number, now: number, to: number, forecasts: ForecastLine[] = []): {rows: ReadoutRow[]; planned: boolean; foreseen: boolean} {
  ...
  return {rows, planned: plans.some(plan => plan.runs.length > 0), foreseen: forecasts.some(forecast => forecast.points.length > 0)};
}
