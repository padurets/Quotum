...
test('static, board and device tokens are told apart; anything else is refused', () => {
  ...
  const boardSecret = newSecret('al_b');
  ...
  assert.equal(ingest.authenticate(`Bearer ${newSecret('al_d')}`), null);
...
test('the first account of a provider takes its default source; others get their own', () => {
  ...
  const ids = store.states().filter(s => s.provider === 'codex').map(s => s.windows[0]?.used);
  assert.deepEqual(ids, [5, 7]);
  assert.equal(store.state('codex').windows[0].used, 5);
  // With a collector writing the default sources, agents never take them.
  const shared = setup(false);
  shared.ingest.accept(STATIC, batch([snapshot(start, 5)]), start);
  assert.deepEqual(shared.store.states().map(s => [s.id.startsWith('codex:'), s.windows[0].used]), [[true, 5]]);
});
...
test('one account measured by several devices is one source', () => {
  ...
  assert.deepEqual([store.states().length, series.samples, series.consumed], [1, 2, 1]);
});
...
test('a subscription the client does not name belongs to its owner, not to the machine', () => {
  ...
  const used = store.states().map(s => s.windows[0].used).sort((a, b) => a - b);
...
test('stored agent samples carry their staleness into history', () => {
  ...
  const history = store.history(DEFAULT_BOARD, start - 1, 300_000).find(s => s.sourceId === 'codex')!;
