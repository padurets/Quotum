...
test('limits back before their reset time and free resets granted are events for the chart', () => {
  ...
  store.record(id, measurement({observedAt: at(0), windows: [weekly(90), session(40)], resets: {available: 0, expiring: []}}));
  store.record(id, measurement({observedAt: at(2), windows: [weekly(91), session(41)], resets: {available: 1, expiring: []}}));
