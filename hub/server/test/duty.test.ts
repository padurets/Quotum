...
test('the first device to ask measures; the others wait and are told when to ask again', () => {
  const duty = new Duty();
  assert.deepEqual(duty.claim('b', 'acc', 'laptop', false, t0), {measure: true, until: t0});
  const waiting = duty.claim('b', 'acc', 'server', false, t0 + 1000);
  assert.equal(waiting.measure, false);
  assert.equal(waiting.until, t0 + 5 * MIN, 'until the holder’s first lease runs out');
  assert.equal(duty.claim('b', 'acc', 'laptop', false, t0 + 2 * MIN).measure, true, 'the holder keeps measuring');
  assert.equal(duty.claim('b', 'other', 'server', false, t0).measure, true, 'another subscription has its own duty');
  assert.equal(duty.claim('team', 'acc', 'server', false, t0).measure, true, 'and so does another board');
});

test('duty sticks while the holder delivers, and passes on when it goes quiet', () => {
  const duty = new Duty();
  duty.claim('b', 'acc', 'laptop', false, t0);
  duty.delivered('b', 'acc', 'laptop', t0 + 10_000, 204_000, t0 + 11_000);
  const lease = t0 + 10_000 + 204_000;
  assert.deepEqual(duty.claim('b', 'acc', 'server', false, t0 + MIN), {measure: false, until: lease});
  // The laptop sleeps: nothing arrives, the lease runs out, the server takes over.
  assert.equal(duty.claim('b', 'acc', 'server', false, lease + 1).measure, true);
  assert.equal(duty.holder('b', 'acc'), 'server');
  assert.equal(duty.claim('b', 'acc', 'laptop', false, lease + MIN).measure, false, 'the old holder now waits');
});

test('a device in use takes duty from an idle holder, never from a busy one', () => {
  const duty = new Duty();
  duty.claim('b', 'acc', 'server', true, t0);
  duty.delivered('b', 'acc', 'server', t0, 18 * MIN, t0);
  // Someone works on the laptop, but the server was in use a minute ago: no flapping.
  const soon = duty.claim('b', 'acc', 'laptop', true, t0 + MIN);
  ...
  assert.equal(duty.claim('b', 'acc', 'laptop', true, t0 + 11 * MIN).measure, true);
...
test('an idle waiting device asks again in at most ten minutes', () => {
  ...
  duty.delivered('b', 'acc', 'server', t0, 18 * MIN, t0);
  assert.deepEqual(duty.claim('b', 'acc', 'laptop', false, t0 + MIN), {measure: false, until: t0 + 11 * MIN});
});
...
test('check-ins are resolved per subscription, the owner’s own ones included', () => {
  ...
  // Same owner, same Claude account, same (unnamed) Antigravity: the second machine waits for both.
