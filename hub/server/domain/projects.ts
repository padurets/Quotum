...
export type ProjectGroup = {
  ...
  /** The reported names it gathers, its own among them, by name; none for work without a project. */
  reported: string[];
};

/** A person's machine that worked under a reported name ('' for none), and when it last did. */
export type ReportedWork = {reported: string; machine: {id: string; name: string}; lastAt: number};
...
/** A person's projects, most recently worked on first, then by name; work without a project last. */
export function projectGroups(work: ReportedWork[], names: Map<string, string>): ProjectGroup[] {
  ...
  const group = (reported: string) => {
    ...
    if (!found) groups.set(name, (found = {name, lastAt: null, machines: [], reported: []}));
    if (name !== null && !found.reported.includes(reported)) found.reported.push(reported);
    return found;
  };
  for (const {reported, machine, lastAt} of work) {
  ...
  // A correction with no work kept is still listed, under the name it gives.
  ...
  return [...groups.values()]
    .map(found => ({
      ...
      reported: found.reported.sort(byName),
    }))
