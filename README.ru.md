...
- **Linux** (x64): пакет вашей системы — он подтянет WebKitGTK и библиотеку трея и
  ...
  рисует через X11, а в новой системе (например, в Fedora) это может значить — без GPU:
  без размытия, с медленной прокруткой или вовсе без запуска. Где есть пакет, берите
  пакет.
