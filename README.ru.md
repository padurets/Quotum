...
(`quotum-desktop-<версия>-<коммит>-linux-x64.deb`, `.rpm`, `.AppImage` и
`…-windows-x64-setup.exe`; чтобы их скачать, нужен аккаунт GitHub). Как собрать самому —
...
- **Linux** (x64): пакет вашей системы — он подтянет WebKitGTK и библиотеку трея и
  уживается с системным `nodejs`. На Debian 12, Ubuntu 22.04 и новее — `sudo apt install
  ./quotum-desktop-<…>.deb`, на Fedora — `sudo dnf install ./quotum-desktop-<…>.rpm`. В
  остальных системах — AppImage: сделайте его исполняемым (`chmod +x`) и запустите (без
  FUSE 2, `libfuse2`, добавьте `--appimage-extract-and-run`). Он несёт свой WebKitGTK и
  рисует через X11; где есть пакет системы, лучше взять его. Для проверенных сочетаний
  NVIDIA и WebKit приложение выбирает совместимый режим, сохраняя GPU-композитинг и
  размытие. Если графика всё же не работает, полностью выйдите из приложения и
  попробуйте `quotum-desktop --software-rendering` (или добавьте этот параметр к
  команде AppImage). Он действует только на этот запуск. Фактический режим графики
  записывается в `hub.log`.
...
- **Запуск при входе** включается сам, когда приложение впервые меряет, и запускает его
  ...
  AppImage по пути: держите его на месте и по пути без пробелов (после переноса
  выключите и снова включите запуск при входе).
- **Новая сборка** ставится поверх старой: сначала выйдите из приложения.
...
- **Что уходит с машины:** ничего, кроме объявлений о сбросах, которые доска читает у
  ...
  есть только у окна.
- **Размер:** установщик для Windows — около 27 МБ, deb и rpm — по 51 МБ (около 140 МБ
  после установки), AppImage — 125 МБ; большая часть — Node.js, на которой работает его хаб.
...
desktop/              десктопное приложение (Tauri): агент, свой хаб и доска в окне
