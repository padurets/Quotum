...
- **Какие агенты на ней запущены и какие работают.** Под лимитами — метка на каждую
  ...
  залитая, пока работает, контурная, пока простаивает; в подсказке проект и сколько
  она уже запущена. Терминалы, редакторы и приложение Codex — одинаково.
...
- **Не мешает.** В простое агент занимает около 5 МБ памяти. Дорого здесь другое —
  ...
  цифры, раз в минуту, когда осталось мало, и реже, пока ничего не происходит, вплоть
  до раза в 15 минут. Машина спрашивает хаб раз в 15 секунд, это ничего не запускает.
- **Доступы остаются на месте.** Quotum никогда не читает, не хранит и не отправляет
  ...
  времени и как называется папка их проекта (`sessions = false` и `projects = false`
  это отключают). Участникам доски их видно вместе с именем машины, только если
  человек, чьи это агенты, показывает эту подписку на доске. Полный список — в [спецификации](spec/ingest-v1.md#privacy).
...
projects = true         # вместе с именами папок их проектов
