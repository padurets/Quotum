...
`main` и в пулл-реквестах и хранит сборки в артефактах прогона
(`quotum-desktop-<версия>-<коммит>-<платформа>`; чтобы их скачать, нужен аккаунт
GitHub). Как собрать самому — в [CONTRIBUTING.md](CONTRIBUTING.md).
...
- **Linux** (x64; Ubuntu 22.04, Debian 12 или новее): `sudo apt install
  ./Quotum_<версия>_amd64.deb` — он подтянет WebKitGTK и библиотеку трея и уживается с
  пакетом `nodejs`; или AppImage: сделайте его исполняемым (`chmod +x`) и запустите
  (без FUSE 2, `libfuse2`, добавьте `--appimage-extract-and-run`).
