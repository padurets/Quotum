...
const weekly = (change: Partial<Win> = {}): Win => ({id: 'weekly', label: 'Неделя', used: 0, remaining: 100, resetAt: start + 7 * DAY, minutes: 10080, ...change});
...
test('short windows are planned linearly to their reset; idle rolling windows have no plan', () => {
  const session: Win = {id: 'session', label: '5 часов', used: 0, remaining: 100, resetAt: start + 5 * 3_600_000, minutes: 300};
